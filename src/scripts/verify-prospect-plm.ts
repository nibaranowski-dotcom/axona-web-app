/**
 * Verify PROSPECT-PLM — the deep PLM golden-thread overlay for a seeded prospect
 * tenant (config management + traceability + BR.1 build-readiness for a multi-factory
 * defense manufacturer). The tenant config is gitignored (SEED.1: its real marque is
 * never committed — this verify is marque-free and resolves the org by a non-marque
 * anchor, the SN-H-4471 serial), so this is an INTEGRATION check. DB checks gate on
 * DATABASE_URL:
...
async function run(): Promise<void> {
  ...
  if (!heroUnit?.orgId) {
    console.log(
      "\n  SKIP real-thread checks — the prospect tenant is not seeded (run its gitignored config)",
    );
  ...
  await check(
    ...
    async () => {
      ...
      return (
        ...
        lockAudit >= 2 &&
        !!persona &&
        [agents, suppliers, parts, pos2, bom2, units, audit, deliveries].every(
          (c) => c > 0,
        )
      );
    },
