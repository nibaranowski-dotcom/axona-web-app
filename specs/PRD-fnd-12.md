...
demo can't show the SERVO-204 → NCR-118 → ECO-318 → Tier-1 Auto OEM-order ripple that proves Axona is a platform,
...
| Narrative chain resolvable | NCR-118 → lot 88421 → ECO-318 → SERVO-205 + fw v4.2.2 → Tier-1 Auto OEM deal → DLV-3312 customs hold all present and cross-linked |
...
- As a **demo driver**, I want one connected dataset so that Command Center, Quality, Engineering,
  Fulfillment, Fleet, Field Service, Finance, and Legal all show the same SERVO/NCR/ECO/Tier-1 Auto OEM story.
...
Procurement real. Use `Tier-1 Auto OEM`/`OEM-2` as sample data **inside the seed** (allowed per §3.7); they must
...
- Anonymizing Tier-1 Auto OEM/OEM-2 — that's the export gate (screen-export-instruction.md), not the seed.
...
async function run() {
  ...
  if (!process.env.DATABASE_URL) { console.log("  SKIP data checks — DATABASE_URL not set (seed + run with docker up)"); }
    ...
    await check("ECO-318 present, affects Tier-1 Auto OEM order", async () => { const e = await db.eCO.findFirst({ where: { code: "ECO-318" } }); return !!e && /Tier-1 Auto OEM/i.test(e.affected); });
...
- [ ] In psql: NCR-118 → linkedTo 'lot 88421'; ECO-318.affected mentions Tier-1 Auto OEM; DLV-3312 stage=CUSTOMS, EAR99 in riskState.
- [ ] Module count = 24; Project = 14; Machine = 21 (8 FIXED).
- [ ] No tenant row exists with an orgId outside the demo/second org.
- [ ] Sample data note: Tier-1 Auto OEM/OEM-2 are illustrative sample data (anonymized only on export).
```
